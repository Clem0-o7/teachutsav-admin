...
export async function GET(req) {
  await dbConnect();
  // Get all users with ideathon submissions
  const users = await User.find({ "submissions.type": "ideathon" }).lean();
  let submissions = [];
  for (const user of users) {
    for (const sub of user.submissions.filter(s => s.type === "ideathon")) {
      let team = null;
      let members = [];
      if (sub.teamId) {
  ...
  // Sort: teams first, then individuals, then by teamName
